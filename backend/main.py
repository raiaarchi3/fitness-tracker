...
import secrets
# ── Config ────────────────────────────────────────────────────────────
SECRET_KEY = os.environ.get("SECRET_KEY", "obsidian-lens-secret-key-change-in-production-2026")
ALGORITHM  = "HS256"
...
EXERCISE_LIBRARY = {
    "Chest":    [
        {"name":"Bench Press",     "category":"COMPOUND / CHEST",    "sets":[{"set":1,"lbs":135,"reps":12},{"set":2,"lbs":185,"reps":8}]},
        {"name":"Incline DB Fly",  "category":"ISOLATION / CHEST",   "sets":[{"set":1,"lbs":45,"reps":15}]},
        {"name":"Cable Crossover", "category":"ISOLATION / CHEST",   "sets":[{"set":1,"lbs":40,"reps":15}]},
        {"name":"Chest Dips",      "category":"COMPOUND / CHEST",    "sets":[{"set":1,"lbs":0,"reps":12}]},
    ],
    "Back":     [
        {"name":"Deadlift",        "category":"COMPOUND / BACK",     "sets":[{"set":1,"lbs":225,"reps":5}]},
        {"name":"Lat Pulldown",    "category":"ISOLATION / BACK",    "sets":[{"set":1,"lbs":120,"reps":12}]},
        {"name":"Bent-Over Row",   "category":"COMPOUND / BACK",     "sets":[{"set":1,"lbs":135,"reps":8}]},
        {"name":"Seated Cable Row","category":"COMPOUND / BACK",     "sets":[{"set":1,"lbs":100,"reps":12}]},
    ],
    "Shoulder": [
        {"name":"Overhead Press",  "category":"COMPOUND / SHOULDER", "sets":[{"set":1,"lbs":95,"reps":8}]},
        {"name":"Lateral Raises",  "category":"ISOLATION / SHOULDER","sets":[{"set":1,"lbs":25,"reps":15}]},
    ...
    "Core":     [
        {"name":"Plank",           "category":"COMPOUND / CORE",     "sets":[{"set":1,"lbs":0,"reps":60}]},
        {"name":"Cable Crunches",  "category":"ISOLATION / ABS",     "sets":[{"set":1,"lbs":50,"reps":15}]},
        {"name":"Leg Raises",      "category":"ISOLATION / ABS",     "sets":[{"set":1,"lbs":0,"reps":20}]},
        {"name":"Russian Twists",  "category":"ISOLATION / OBLIQUES","sets":[{"set":1,"lbs":25,"reps":20}]},
    ],
}

# ── Workout split ─────────────────────────────────────────────────────
WORKOUT_SPLIT = {0:"Rest",1:"Chest",2:"Back",3:"Shoulder",4:"Arms",5:"Legs",6:"Core"}

# ── Routes ────────────────────────────────────────────────────────────

@app.get("/")
def root():
    return {"status": "Obsidian Lens API", "version": "2.0"}
...
def register(data: RegisterRequest):
    ...
    c = conn.execute(
        "INSERT INTO users (email, password_hash, name, setup_complete) VALUES (?,?,?,0)",
    ...
    return {"token": token, "user": dict(user), "setup_complete": False}
...
def login(data: LoginRequest):
    ...
    return {"token": token, "user": dict(user), "setup_complete": bool(user["setup_complete"])}

@app.post("/auth/setup")
def complete_setup(data: SetupRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    conn.execute(
        "UPDATE users SET name=?, weight=?, height=?, age=?, goal=?, activity=?, setup_complete=1 WHERE id=?",
        (data.name, data.weight, data.height, data.age, data.goal, data.activity, current_user["id"])
    )
    conn.commit()
    user = conn.execute("SELECT * FROM users WHERE id=?", (current_user["id"],)).fetchone()
    conn.close()
    return {"user": dict(user), "setup_complete": True}
...
@app.put("/user")
def update_user(data: UserUpdate, current_user=Depends(get_current_user)):
    conn = get_db()
    fields = {k: v for k, v in data.dict().items() if v is not None}
    if fields:
        sets = ", ".join(f"{k}=?" for k in fields)
...
def get_today(current_user=Depends(get_current_user)):
    ...
    muscle = WORKOUT_SPLIT.get((dow + 1) % 7, "Rest")
    conn  = get_db()
    water = conn.execute("SELECT SUM(amount_ml) as total FROM water_logs WHERE user_id=? AND date=?", (uid, today)).fetchone()
    nuts  = conn.execute("SELECT SUM(calories) as cal, SUM(protein) as pr, SUM(carbs) as cb, SUM(fats) as ft FROM nutrient_logs WHERE user_id=? AND date=?", (uid, today)).fetchone()
    sess  = conn.execute("SELECT * FROM workout_sessions WHERE user_id=? AND date=? ORDER BY id DESC LIMIT 1", (uid, today)).fetchone()
    conn.close()
    return {
        "today":             today,
        ...
        "user":              current_user,
        "water_ml":          water["total"] or 0,
        "water_goal_ml":     round((current_user.get("weight", 70) * 0.035 + 0.5) * 1000),
        "calories":          nuts["cal"] or 0,
        "protein":           round(nuts["pr"] or 0, 1),
        "carbs":             round(nuts["cb"] or 0, 1),
        "fats":              round(nuts["ft"] or 0, 1),
        "session_id":        sess["id"] if sess else None,
        "session_completed": bool(sess["completed"]) if sess else False,
    }

# ── SESSIONS ──────────────────────────────────────────────────────────

@app.post("/sessions/start")
def start_session(data: WorkoutSession, current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = data.date or str(date.today())
    conn  = get_db()
    existing = conn.execute("SELECT id FROM workout_sessions WHERE user_id=? AND date=? AND muscle_group=?", (uid, today, data.muscle_group)).fetchone()
    if existing:
        session_id = existing["id"]
    else:
        c = conn.execute("INSERT INTO workout_sessions (user_id, muscle_group, date) VALUES (?,?,?)", (uid, data.muscle_group, today))
        session_id = c.lastrowid
        for ex in EXERCISE_LIBRARY.get(data.muscle_group, []):
            conn.execute("INSERT INTO exercises (session_id, name, category, muscle_group, sets_data, completed) VALUES (?,?,?,?,?,0)",
                         (session_id, ex["name"], ex["category"], data.muscle_group, json.dumps(ex["sets"])))
        conn.commit()
    session   = conn.execute("SELECT * FROM workout_sessions WHERE id=?", (session_id,)).fetchone()
    exercises = conn.execute("SELECT * FROM exercises WHERE session_id=?", (session_id,)).fetchall()
    exs = []
    for e in exercises:
        d = dict(e); d["sets_data"] = json.loads(d["sets_data"]); exs.append(d)
    conn.close()
    return {"session": dict(session), "exercises": exs}
...
def get_session(session_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    session = conn.execute("SELECT * FROM workout_sessions WHERE id=? AND user_id=?", (session_id, current_user["id"])).fetchone()
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    exercises = conn.execute("SELECT * FROM exercises WHERE session_id=?", (session_id,)).fetchall()
    exs = []
...
@app.post("/sessions/{session_id}/exercises")
def add_exercise(session_id: int, data: ExerciseUpdate, current_user=Depends(get_current_user)):
    conn = get_db()
    session = conn.execute("SELECT * FROM workout_sessions WHERE id=? AND user_id=?", (session_id, current_user["id"])).fetchone()
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    conn.commit()
    ex = conn.execute("SELECT * FROM exercises WHERE id=?", (c.lastrowid,)).fetchone()
    d = dict(ex); d["sets_data"] = json.loads(d["sets_data"])
    conn.close()
    return d

@app.put("/exercises/{exercise_id}")
def update_exercise(exercise_id: int, data: ExerciseUpdate, current_user=Depends(get_current_user)):
    conn = get_db()
    conn.execute("UPDATE exercises SET name=?, category=?, sets_data=?, completed=? WHERE id=?",
                 (data.name, data.category, json.dumps(data.sets_data), int(data.completed), exercise_id))
    conn.commit()
...
@app.delete("/exercises/{exercise_id}")
def delete_exercise(exercise_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    conn.execute("DELETE FROM exercises WHERE id=?", (exercise_id,))
    conn.commit()
    conn.close()
...
@app.post("/sessions/{session_id}/complete")
def complete_session(session_id: int, data: SessionComplete, current_user=Depends(get_current_user)):
    uid  = current_user["id"]
    conn = get_db()
    conn.execute("UPDATE workout_sessions SET completed=1, duration_seconds=? WHERE id=? AND user_id=?", (data.duration_seconds, session_id, uid))
    today     = str(date.today())
    yesterday = str(date.today() - timedelta(days=1))
    ...
    if last == yesterday:
        ...
        streak = 1
    conn.execute("UPDATE users SET streak=?, last_workout_date=? WHERE id=?", (streak, today, uid))
    conn.commit()
    conn.close()
    return {"completed": True, "streak": streak}
...
def get_session_history(current_user=Depends(get_current_user)):
    conn = get_db()
    rows = conn.execute("SELECT * FROM workout_sessions WHERE user_id=? ORDER BY date DESC LIMIT 30", (current_user["id"],)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
@app.post("/water")
def log_water(data: WaterLog, current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = data.date or str(date.today())
    conn  = get_db()
    existing = conn.execute("SELECT id, amount_ml FROM water_logs WHERE user_id=? AND date=?", (uid, today)).fetchone()
    if existing:
        new_total = min(existing["amount_ml"] + data.amount_ml, 6000)
        conn.execute("UPDATE water_logs SET amount_ml=? WHERE id=?", (new_total, existing["id"]))
    else:
        new_total = data.amount_ml
        conn.execute("INSERT INTO water_logs (user_id, date, amount_ml) VALUES (?,?,?)", (uid, today, new_total))
    conn.commit()
    conn.close()
    return {"date": today, "amount_ml": new_total}

@app.get("/water/week")
def get_water_week(current_user=Depends(get_current_user)):
    conn = get_db()
    rows = conn.execute("SELECT date, amount_ml FROM water_logs WHERE user_id=? ORDER BY date DESC LIMIT 7", (current_user["id"],)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
@app.post("/nutrients")
def log_nutrient(data: NutrientLog, current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = data.date or str(date.today())
    conn  = get_db()
    conn.execute("INSERT INTO nutrient_logs (user_id, date, food_name, calories, protein, carbs, fats, fiber, macro_category) VALUES (?,?,?,?,?,?,?,?,?)",
                 (uid, today, data.food_name, data.calories, data.protein, data.carbs, data.fats, data.fiber, data.macro_category))
    conn.commit()
    logs = conn.execute("SELECT * FROM nutrient_logs WHERE user_id=? AND date=? ORDER BY created_at DESC", (uid, today)).fetchall()
    conn.close()
    return [dict(r) for r in logs]

@app.get("/nutrients/today")
def get_nutrients_today(current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = str(date.today())
    conn  = get_db()
    logs   = conn.execute("SELECT * FROM nutrient_logs WHERE user_id=? AND date=? ORDER BY created_at DESC", (uid, today)).fetchall()
    totals = conn.execute("SELECT SUM(calories) as cal, SUM(protein) as pr, SUM(carbs) as cb, SUM(fats) as ft, SUM(fiber) as fb FROM nutrient_logs WHERE user_id=? AND date=?", (uid, today)).fetchone()
    conn.close()
    return {
        "logs":   [dict(r) for r in logs],
        "totals": {"calories": totals["cal"] or 0, "protein": round(totals["pr"] or 0, 1), "carbs": round(totals["cb"] or 0, 1), "fats": round(totals["ft"] or 0, 1), "fiber": round(totals["fb"] or 0, 1)}
    }

@app.delete("/nutrients/{nutrient_id}")
def delete_nutrient(nutrient_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    conn.execute("DELETE FROM nutrient_logs WHERE id=? AND user_id=?", (nutrient_id, current_user["id"]))
    conn.commit()
    conn.close()
    return {"deleted": True}
...
@app.post("/weight")
def log_weight(data: WeightLog, current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = data.date or str(date.today())
    conn  = get_db()
    conn.execute("INSERT OR REPLACE INTO weight_logs (user_id, date, weight) VALUES (?,?,?)", (uid, today, data.weight))
    conn.execute("UPDATE users SET weight=? WHERE id=?", (data.weight, uid))
    conn.commit()
    conn.close()
    return {"date": today, "weight": data.weight}

@app.get("/weight/history")
def get_weight_history(current_user=Depends(get_current_user)):
    conn = get_db()
    rows = conn.execute("SELECT date, weight FROM weight_logs WHERE user_id=? ORDER BY date DESC LIMIT 14", (current_user["id"],)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
@app.post("/study")
def log_study(data: StudyLog, current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = data.date or str(date.today())
    conn  = get_db()
    conn.execute("INSERT INTO study_logs (user_id, date, subject, seconds, note, mode) VALUES (?,?,?,?,?,?)",
                 (uid, today, data.subject, data.seconds, data.note or "", data.mode))
    conn.commit()
    conn.close()
    return {"logged": True}

@app.get("/study/today")
def get_study_today(current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    today = str(date.today())
    conn  = get_db()
    logs  = conn.execute("SELECT * FROM study_logs WHERE user_id=? AND date=? ORDER BY created_at DESC", (uid, today)).fetchall()
    total = conn.execute("SELECT SUM(seconds) as total FROM study_logs WHERE user_id=? AND date=?", (uid, today)).fetchone()
    conn.close()
    return {"logs": [dict(r) for r in logs], "total_seconds": total["total"] or 0}

@app.get("/study/week")
def get_study_week(current_user=Depends(get_current_user)):
    uid      = current_user["id"]
    week_ago = str(date.today() - timedelta(days=7))
    conn     = get_db()
    rows = conn.execute("SELECT date, subject, SUM(seconds) as seconds FROM study_logs WHERE user_id=? AND date>=? GROUP BY date, subject ORDER BY date DESC", (uid, week_ago)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_weekly_analytics(current_user=Depends(get_current_user)):
    ...
    week_ago = str(date.today() - timedelta(days=7))
    conn     = get_db()
    sessions  = conn.execute("SELECT date, muscle_group, completed, duration_seconds FROM workout_sessions WHERE user_id=? AND date>=? ORDER BY date ASC", (uid, week_ago)).fetchall()
    water     = conn.execute("SELECT date, amount_ml FROM water_logs WHERE user_id=? AND date>=? ORDER BY date ASC", (uid, week_ago)).fetchall()
    nutrients = conn.execute("SELECT date, SUM(calories) as cal, SUM(protein) as pr FROM nutrient_logs WHERE user_id=? AND date>=? GROUP BY date ORDER BY date ASC", (uid, week_ago)).fetchall()
    user      = conn.execute("SELECT streak, weight FROM users WHERE id=?", (uid,)).fetchone()
    conn.close()
    return {"sessions": [dict(s) for s in sessions], "water": [dict(w) for w in water], "nutrients": [dict(n) for n in nutrients], "streak": user["streak"] if user else 0, "current_weight": user["weight"] if user else 0}

@app.get("/analytics/streak-calendar")
def get_streak_calendar(current_user=Depends(get_current_user)):
    uid   = current_user["id"]
    start = str(date.today() - timedelta(days=84))
    conn  = get_db()
    rows  = conn.execute("SELECT date, completed FROM workout_sessions WHERE user_id=? AND date>=? ORDER BY date ASC", (uid, start)).fetchall()
    conn.close()
    calendar = {}
    for r in rows:
        d = r["date"]
...
@app.get("/stats/overview")
def get_stats_overview(current_user=Depends(get_current_user)):
    uid  = current_user["id"]
    conn = get_db()
    total_sessions  = conn.execute("SELECT COUNT(*) FROM workout_sessions WHERE user_id=?", (uid,)).fetchone()[0]
    total_completed = conn.execute("SELECT COUNT(*) FROM workout_sessions WHERE user_id=? AND completed=1", (uid,)).fetchone()[0]
    total_water     = conn.execute("SELECT SUM(amount_ml) FROM water_logs WHERE user_id=?", (uid,)).fetchone()[0] or 0
    user            = conn.execute("SELECT streak FROM users WHERE id=?", (uid,)).fetchone()
    try:
        total_study = conn.execute("SELECT SUM(seconds) FROM study_logs WHERE user_id=?", (uid,)).fetchone()[0] or 0
    except:
        total_study = 0
    conn.close()
    return {
